...
def load_all():
    ...
    try:
        with open('feature_names.pkl', 'rb') as f:
            assets['features'] = pickle.load(f)
    except:
        assets['features'] = [f"Col_{i}" for i in range(95)]
    ...
    try: 
        m = MLP(95)
        m.load_state_dict(torch.load('mlp_model.pth', map_location=device))
        m.eval()
        models['mlp'] = m
    except: pass
    ...
    try:
        m = CNN1D()
        m.load_state_dict(torch.load('cnn_model.pth', map_location=device), strict=False)
        m.eval()
        models['cnn'] = m
    except: pass
    ...
    try:
        m = RNNModel('lstm')
        m.load_state_dict(torch.load('lstm_model.pth', map_location=device), strict=False)
        m.eval()
        models['lstm'] = m
    except: pass
...
with tab1:
    ...
    if col_d1.button("🟢 Generate Healthy Company"):
        # Create random healthy data (higher values generally better in this dataset)
        data = np.random.rand(1, 95) * 0.5 + 0.5 
...
if st.session_state.input_df is not None:
    ...
    if st.button("🚀 Run Risk Assessment Protocol", type="primary"):
        with st.spinner("Running Hybrid Ensemble Inference..."):
            ...
            if 'scaler' in assets:
                try: 
                    X_scaled = assets['scaler'].transform(input_row).astype(np.float32)
                except: 
            ...
            # Sklearn Inference
            if 'xgb' in models: 
                probs['XGBoost'] = models['xgb'].predict_proba(X_scaled)[:, 1][0]
            if 'rf' in models: 
                probs['Random Forest'] = models['rf'].predict_proba(X_scaled)[:, 1][0]
            
            # Deep Learning Inference
            with torch.no_grad():
                if 'mlp' in models: 
                    probs['Deep MLP'] = F.softmax(models['mlp'](X_tensor), dim=1)[:, 1].item()
                if 'cnn' in models: 
                    probs['1D-CNN'] = F.softmax(models['cnn'](X_cnn), dim=1)[:, 1].item()
                if 'lstm' in models: 
                    probs['LSTM'] = F.softmax(models['lstm'](X_rnn), dim=1)[:, 1].item()

            # 3. Weighted Average (Fallback to simple average if weights missing)
            final_score = np.mean(list(probs.values())) if probs else 0
            ...
            with col_res2:
                ...
                if probs:
                    ...
                    st.warning("No models loaded. Please check the model files.")
